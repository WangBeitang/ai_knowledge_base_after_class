...
4. [x] 9.3.20 已完成：无卡 preflight（运行前检查）通过，正式 GPU（图形处理器）
   完整执行 25/25 条；旧 `8/25` 校正为新 `9/25`。Evaluator（评测器）v2 将归因修正为
   13 条持续模型失败、3 条真实回放新暴露、4 条旧 Provider（动作执行器）误判修正和
   5 条不变通过；8 条回放缺口保留为独立执行状态，不再覆盖模型路线责任。
5. 待用户明确启动 9.3.21；只按 16 条模型路线失败推进，不因回放缺口补训练数据。
   在准入通过前不运行 heldout（留出集）、不进入 9.4。
