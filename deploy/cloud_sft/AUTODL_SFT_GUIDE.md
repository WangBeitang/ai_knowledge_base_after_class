...
`eligible_for_stage9_4=false`（不直接允许进入 9.4），且 heldout test（留出测试）
仍不得运行。2026-07-31 的正式结果已人工确认：新路线正确 `9/25`，16 条模型路线失败；
其中8条同时存在回放缺口，但模型在缺口前已选错路线，因此不需要重新运行 GPU
（图形处理器）。9.3.21 只在用户明确启动后继续。
