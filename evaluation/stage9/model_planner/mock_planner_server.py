"""阶段 9 PlannerModelServer（规划器模型服务）mock（模拟）入口。"""
...
MOCK_SERVER_VERSION = "stage9-mock-planner-server-v1"
...
def make_handler(decision: PlannerDecision) -> type[BaseHTTPRequestHandler]:
    ...
    class MockPlannerHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            response = {
                "id": "stage9-mock-planner",
                "object": "chat.completion",
                "model": str(request_payload.get("model") or "stage9-mock"),
                "choices": [
                    {
                        "index": 0,
                        "message": {
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="阶段 9 mock PlannerModelServer。")
