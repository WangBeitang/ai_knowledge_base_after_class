...
脚本只比较本地检索 schema，不执行 Web，也不训练或修改 Planner。评测病例必须由真实
...
from app.rag.query.contracts import RetrievalMode, SubjectResolutionStatus  # noqa: E402
...
class RetrievalEvalCase(BaseModel):
    ...
    query_id: str = Field(min_length=1)  # 稳定病例 ID，开发集和 held-out 集不能重复。
    ...
    expected_identifier_resolution_status: str  # exact_match、suggestion_required 等期望状态。
    ...
    def require_relevance_labels(self):
        if self.should_answer and not self.expected_chunk_ids:
            raise ValueError("should_answer=true 的病例必须标注 expected_chunk_ids")
...
def load_cases(path: Path, *, allow_small_set: bool = False) -> list[RetrievalEvalCase]:
    ...
    if not allow_small_set and not 24 <= len(cases) <= 40:
        raise ValueError("正式阶段 5 专项评测集必须包含 24～40 条病例")
...
def metrics_for_ranking(expected_chunk_ids: list[str | int], retrieved_chunk_ids: list[str | int]) -> dict[str, float]:
    ...
    if not expected:
        return {"recall_at_k": 1.0 if not retrieved else 0.0, "mrr": 0.0, "ndcg": 0.0}
...
def summarize_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    """按 retrieval mode 汇总质量与延迟；资源成本等待真实建库记录后补入报告。"""
...
def run(cases: list[RetrievalEvalCase]) -> dict[str, Any]:
    results = [
