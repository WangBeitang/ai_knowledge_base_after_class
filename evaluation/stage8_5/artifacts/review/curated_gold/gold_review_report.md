...
- `schema_approved_cases.jsonl` 只代表阶段 8.5 基础门禁放行，不等于 Gold/domain verified。
...
## 建议
- 不要把当前 24 条 `schema_approved_cases.jsonl` 直接当 Gold 使用。
...
- `gold_cases_authoring.jsonl` 中不再包含原候选的维修动作、未公开根因或通用安全建议。
