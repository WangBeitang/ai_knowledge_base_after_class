...
为什么不能直接把 ``gold_cases_authoring.jsonl`` 交给快照构建器：
...
def build_runtime_artifacts(
    ...
    if set(audit_by_case) != {case.case_id for case in cases}:
        raise ValueError("gold_case_audit.jsonl 与 gold_cases_authoring.jsonl 的 case_id 集合不一致")
    verified_audits = [
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence_path = layout.curated_intermediate / "gold_evidence_chunks.jsonl"
    source_cases_path = layout.curated_intermediate / "gold_cases_authoring.jsonl"
    ...
    indexed_cases_path = layout.curated_intermediate / "gold_cases_indexed.jsonl"
    runtime_bindings_path = layout.curated_intermediate / "gold_case_runtime_bindings.jsonl"
    import_manifest_path = layout.curated_intermediate / "gold_import_manifest.json"
    split_manifest_path = layout.curated_intermediate / "gold_split_manifest.json"
    snapshot_path = layout.curated_intermediate / "environment_snapshot_import_v1.json"
    report_path = layout.reports / "阶段8.5Gold证据导入与快照报告.md"
    ...
    markdown_paths = _materialize_markdown_documents(
        prepared_documents,
        layout.curated_intermediate / "gold_documents",
    )
