...
- schema-approved/review/rejected 三个池分开写。schema-approved 只表示通过基础数据门禁，
  不代表已经成为 Gold 或可直接训练，因此文件名不再使用含糊的 approved_cases。
"""
...
from evaluation.stage8_5.pipelines.public_candidate.generate_stage85_report import (  # noqa: E402
    build_markdown_report,
)


DEFAULT_BASE_DIR = DEFAULT_STAGE85_ROOT
...
class CardSeed:
    ...
    两条候选问题已经经过第一轮人工抽查，可以进入 schema-approved 池；False 时只进入 review
    queue，后续人工确认前不能导出训练。
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    split_manifest = build_split_manifest(
        ...
        notes=(
            "阶段 8.5 第一批公开数据 schema-approved case split 清单；"
...
def _stage85_paths(base_dir: Path) -> dict[str, Path]:
    """集中定义公开候选流水线产物，明确中间数据与审核结果的所有权。"""

    layout = stage85_layout(base_dir)
    return {
        "licenses": layout.public_intermediate / "sources/license_manifest.jsonl",
        "sources": layout.public_intermediate / "sources/source_manifest.jsonl",
        "raw_cards": layout.public_intermediate / "fault_scenario_cards.raw.jsonl",
        "cards": layout.public_intermediate / "fault_scenario_cards.jsonl",
        "documents": layout.public_intermediate / "public_documents_manifest.jsonl",
        "chunk_map": layout.public_intermediate / "chunk_source_map.jsonl",
        "candidates": layout.public_intermediate / "planner_case_candidates.jsonl",
        # 名称刻意使用 schema_approved：这 24 条只通过基础门禁，不是正式 Gold。
        "approved": layout.public_review / "schema_approved_cases.jsonl",
        "review": layout.public_review / "review_queue.jsonl",
        "rejected": layout.public_review / "rejected_cases.jsonl",
        "split_manifest": layout.public_review / "split_manifest.json",
        "quality_report": layout.public_intermediate / "data_quality_report.json",
        "markdown_report": layout.reports / "阶段8.5数据处理报告.md",
    }
...
def _chunk_source_records(cards: list[FaultScenarioCard]) -> list[dict[str, Any]]:
    """生成 chunk 到来源卡片的映射，满足 schema-approved case 可追溯要求。"""
