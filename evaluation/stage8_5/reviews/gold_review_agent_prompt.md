...
项目先生成了 52 条公开数据候选，随后发现这些候选混入了来源未直接支撑的维修动作和根因。当前已基于 UCI 官方规则与 profile 标签重写 20 条独立 gold。注意：`approved_cases.jsonl` 仍只代表旧流程放行；本轮主要复核目标是 `gold_cases.jsonl`。

## 需要阅读的文件

- `evaluation/stage8_5/sources/source_manifest.jsonl`：公开来源清单，每行一个 source，包含 `source_id`、标题、URL、许可证状态和备注。
- `evaluation/stage8_5/sources/license_manifest.jsonl`：许可证清单，用来判断来源是否允许训练或再分发派生数据。
- `evaluation/stage8_5/processed/fault_scenario_cards.jsonl`：故障场景卡片。它是原始公开资料和 `PlannerEvalCase` 之间的中间层。
- `evaluation/stage8_5/processed/public_documents_manifest.jsonl`：本地卡片化摘要被视作哪些文档，每条 document 对应哪个 source。
- `evaluation/stage8_5/processed/chunk_source_map.jsonl`：每个 expected chunk 对应哪个 source/card/document。
- `evaluation/stage8_5/candidates/planner_case_candidates.jsonl`：完整 52 条候选 case。
- `evaluation/stage8_5/candidates/approved_cases.jsonl`：当前流程标记为 reviewed 的样本，但不等于 gold。
- `evaluation/stage8_5/candidates/review_queue.jsonl`：当前流程中仍待审核的样本。
- `evaluation/stage8_5/reviews/gold_review_decisions.jsonl`：已有一轮高置信 gold 审核结果，可作为参考，但你需要独立判断。
- `evaluation/stage8_5/processed/gold_evidence_documents.jsonl`：2 个 gold 证据文档清单；当前状态是待导入，不代表已经进入 Milvus。
- `evaluation/stage8_5/processed/gold_evidence_chunks.jsonl`：10 个来源证据 chunk，包含 UCI URL、页面定位、中文证据摘要和原子事实 `fact_id`。
- `evaluation/stage8_5/candidates/gold_cases.jsonl`：本轮主要审核对象，共 20 条重写后的 `PlannerEvalCase`。
- `evaluation/stage8_5/reviews/gold_case_audit.jsonl`：20 条逐点审计记录，包含旧 case 映射、答案点到 fact ID 的映射和已删除内容。
- `evaluation/stage8_5/reviews/gold_rewrite_report.md`：本轮重写数量、覆盖范围和运行边界摘要。
...
1. 逐条读取 `gold_cases.jsonl`；不要因为文件名叫 gold 就直接判为 gold。
