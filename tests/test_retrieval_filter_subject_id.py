...
def test_embedding_search_filters_by_subject_id_only(monkeypatch):
    ...
    assert fake_gateway.exprs == ['subject_id in ["subject_hak_180"]']
    assert result[0]["chunk_id"] == 1001
    assert result[0]["type"] == "milvus"


def test_hyde_search_filters_by_subject_id_only(monkeypatch):
    fake_gateway = FakeMilvusGateway()
